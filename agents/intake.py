    ...
    for attempt in range(3):
        try:
            ...
            if _is_rate_limit(exc):
                ...
                print(f"[INTAKE] Rate limited — retrying in {wait}s (attempt {attempt + 1}/3)")
                time.sleep(wait)
                last_exc = exc
                continue
...
def intake_node(state: AgentState, llm: BaseChatModel) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "messages": [f"[INTAKE] Parsed {len(all_resources)} resource(s) ({len(provider_resources)} provider block(s))"]
        }
