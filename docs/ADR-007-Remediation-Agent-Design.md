...
The current ADAG pipeline terminates at the **Auditor** node. When violations are found, the `Violation` model surfaces a `remediation_hint` (a single free-text string). This is intentionally shallow — it describes _what_ is wrong, not _how to fix it_ in machine-applicable form.
